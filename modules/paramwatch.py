...
def send_discord_notification(new_urls, source_domain):
    ...
    for chunk in chunks:
        message = f"🚨 **New URLs found for {source_domain}** 🚨\n"
        message += f"Total new URLs: {len(new_urls)}\n\n"
    ...
    while True:
        try:
            ...
            for url in urls:
                ...
                if new_urls:
                    print(f"Found {len(new_urls)} new URLs for {domain}")
                    saved = save_new_urls(new_urls, domain)
                    print(f"Saved {saved} new URLs")
                    send_discord_notification(new_urls, domain)
                    total_new += saved

            print_verbose(f"\nCycle complete. Found {total_new} total new URLs.")
